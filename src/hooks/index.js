...
export const useNotes = selectedBoard =>
{
    ...
    useEffect(() =>
    {
        ...
        unsubscribe = selectedBoard && !collatedNotesExist(selectedBoard) ? (unsubscribe = unsubscribe.where('boardId', '==', selectedBoard))
            : selectedBoard === 'Today'
        ...
        unsubscribe = unsubscribe.onSnapshot(snapshot =>
        {
            ...
            setNotes(
                selectedBoard === 'InProgress'
                    ? newNotes.filter(note => moment(note.date, 'DD-MM-YY').diff(moment(), 'days') <= 7 && note.archived !== true)
